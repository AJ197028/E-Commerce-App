...
const CartProvider = ({ children }) => {
    ...
    const fetchCartItems = async () => {
      try {
        const response = await fetch(
          `https://react-http-8b260-default-rtdb.firebaseio.com/cart${emailId}.json`
        );
        if (!response.ok) {
  ...
  const addToCart = async (item) => {
    ...
    if (itemIndex === -1) {
      ...
      try {
        const response = await fetch(`https://react-http-8b260-default-rtdb.firebaseio.com/cart${emailId}/${newItem.uid}.json`,
          {
            method: "PUT",
      ...
      try {
        const response = await fetch(
          `https://react-http-8b260-default-rtdb.firebaseio.com/cart${emailId}/${updatedCartItem.uid}.json`,
          {
            method: "PUT",
  ...
  const removeFromCart = async (itemUid) => {
    ...
    try {
      const response = await fetch(`https://react-http-8b260-default-rtdb.firebaseio.com/cart${emailId}/${itemUid}.json`, {
        method: 'DELETE',
      });
      if (!response.ok) {
